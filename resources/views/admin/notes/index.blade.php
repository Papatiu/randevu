                        ...
                        <form id="note-form">
                            ...
                            <div class="d-grid gap-2">
                                ...
                                <button type="button" class="btn btn-secondary" id="reset-form-btn"
                                    style="display: none;"><i class="fas fa-plus-circle me-2"></i>Yeni Not Ekle</button>
                            </div>
                        </form>
                    </div>
                </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle">
                                ...
                                <tbody id="notes-table-body">
                                    ...
                                    @empty
                                        <tr id="no-notes-row">
                                            <td colspan="3" class="text-center text-muted">Henüz oluşturulmuş bir not
                                                yok.</td>
                                        </tr>
                                    @endforelse
                                </tbody>
...
@section('scripts')
    // HATALI KODUN TAMAMI YERİNE AŞAĞIDAKİ GELECEK

@section('scripts')
    <script>
        // Sayfa tamamen yüklendikten sonra tüm işlemleri başlat.
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const noNotesRow = document.getElementById('no-notes-row'); // "Not yok" satırını bulalım

            // Formu temizle ve "Yeni Ekle" moduna geçir.
            function resetForm() {
                form.reset();
                noteIdInput.value = '';
                // Editor varsa içeriğini temizle
                if (editor) {
                    editor.setData('');
                }
            ...
            form.addEventListener('submit', function(event) {
                ...
                const data = {
                    // Editor'ün verisini güvenle al
                    content: editor ? editor.getData() : document.getElementById('content').value,
                ...
                fetch(url, {
                    ...
                    .then(result => {
                        if (result.success) {
                            ...
                            // Eğer "Henüz not yok" satırı varsa, onu kaldır
                            if (noNotesRow) noNotesRow.remove();

                            // ID varsa, mevcut satırı güncelle. Yoksa, yeni satırı başa ekle.
                            const existingRow = document.getElementById(`note-row-${id}`);
                            if (existingRow) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Ağ Hatası!',
                            text: 'Sunucuya bağlanılamadı: ' + err
                        });
                    });
            });
            ...
            tableBody.addEventListener('click', function(event) {
                // Sadece butonlara tıklanınca çalışsın
